...
set @TotalRecordsInUnitTestBefore = ( select COUNT(*) from performancestatic_imported )
...
set @TotalCountOfFiles = ( select COUNT(*) from #FileList where Line like 'procoutput PrepBatchTransfer_PerformanceStatic%' )
...
-- select * from performancestatic_imported
-- select * from deletethis_performancestatic_imported
if object_id('deletethis_performancestatic_imported') is not null 
	drop table deletethis_performancestatic_imported

-- -------------------
-- create deletethis tables
select * into deletethis_performancestatic_imported from dbo.performancestatic_imported where  1 = 0
--select * into deletethis_GWP_Contact_Addresses from dbo.GWP_Contact_Addresses where  1 = 0
	
ALTER TABLE deletethis_performancestatic_imported 
...
SET @getSourceFile = CURSOR FOR
	SELECT * FROM   #FileList  where Line like 'procoutput PrepBatchTransfer_PerformanceStatic%' ORDER BY Line asc
OPEN @getSourceFile
	FETCH NEXT
...
-- truncate table performancestatic_imported
		set nocount on
		set nocount OFF
		--set @FileRootName = ''
		...
		set @TableRootName = 'performancestatic_imported'
...
--			set @FileRootName = 'IPC_Contact_Addresses'
		...
		BEGIN
			...
			truncate table deletethis_performancestatic_imported
			--select * from deletethis_performancestatic_imported
			declare @sql varchar(max)

			set @sql = '
			BULK INSERT dbo.deletethis_performancestatic_imported
			...
			-- insert into deletethis_performancestatic_imported (TemplateQuery,InitDatetime
			-- select * from deletethis_performancestatic_imported
			
			--SET @sql = ' ' +
			--	' delete performancestatic_imported ' +
			--	' from performancestatic_imported A ' +
			--	' , deletethis_performancestatic_imported B ' +
			...
			SET @sql = ' ' +
			' INSERT INTO performancestatic_imported ( 
			...
			' FROM deletethis_performancestatic_imported '
...
--delete from performancestatic_imported
...
DECLARE @rc_performancestatic_imported int
SET @rc_performancestatic_imported = ( SELECT count(*) FROM performancestatic_imported )
--DECLARE @rc_GWP_Contact_Addresses int
--SET @rc_GWP_Contact_Addresses = ( SELECT count(*) FROM GWP_Contact_Addresses )
PRINT 'Process completed successfully'
PRINT '@rc_performancestatic_imported='+converT(varchar,@rc_performancestatic_imported)

--PRINT '@rc_GWP_Contact_Addresses='+converT(varchar,@rc_GWP_Contact_Addresses)
DECLARE @recordsloaded int
set @recordsloaded = (SELECT count(*) from performancestatic_imported)

set @notes2 = @notes2 + '
Finish Datetime : ' + convert(varchar,getdate())

select convert(varchar,@recordsloaded) + ' records loaded to performancestatic_imported ' + @notes2 
print '.......printing notes.......'
print @notes2
/*

	select * from deletethis_performancestatic_imported
	select * from performancestatic_imported where Portcode = 'SASMO1'
	
	select distinct ManagerStrategy from performancestatic_imported
*/
