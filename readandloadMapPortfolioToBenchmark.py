            ...
            for nodeA in soup.find_all('tr', attrs={}):
                ...
                for nodeB in nodeA.find_all('td', attrs={}):
                    ...
                    if nodeB.attrs['class'][0] in ['DataCellNumeric','DataCellString']:
                        #print 'got here!!!'
                        if fieldnames[len(fieldvalues)] == 'portfolio_ext':
                            is_dataline = 1
                ...
                if is_dataline == 1:
                    ...
                    if already_ran_delete_statement == 0:
                        cursor.execute("truncate table dbo.xanalysisofbenchmarks_mapportfoliotobenchmark" )
                        total_deleted = total_deleted + cursor.rowcount
                        print '  ',cursor.rowcount, 'records deleted'
                        cnxn.commit()
                        already_ran_delete_statement = 1
                    insert_sql = "insert into xanalysisofbenchmarks_mapportfoliotobenchmark("+fieldnames_string+") values ("+fieldvalues_string+")"
                    #print insert_sql
...
if __name__=='__main__':
    ...
    myDataFilePathName = r"//Ipc-vsql01/data/Batches/prod/WatchFolder/incoming/PagesOutput_MapPortfolioToBenchmark_2017-01-09 142254130.xls"
