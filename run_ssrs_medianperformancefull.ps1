...
$newfile= "\\ipc-vsql01\data\Batches\prod\Performance\output\Median_Performance_Full_"+$maxdate+".xls"
#$newfile= "\\ipc-vsql01\data\Batches\prod\Performance\output\Median_Performance_Full_2017-01-10.xls"
write-host $newfile
##############################################################################

Copy-Item "C:\Batches\AutomationProjects\Watcher\output\Median Performance Full.xls" $newfile
Start-Sleep -s 10
...
Add-PSSnapin SqlServerCmdletSnapin100 # here lives Invoke-SqlCmd
Add-PSSnapin SqlServerProviderSnapin100
#$conditions = @("myfilename='$newfile'",'Period=MTD')
$DBParam1 = 'myfilename='+$newfile+''
$DBParam2 = 'Period=MTD'
$DBParam3 = 'DummyText=Hello'
$DBParams = $DBParam1, $DBParam2, $DBParam3
Invoke-sqlcmd -inputfile "$ScriptPath\send-email_medianperformancefull.sql" -Variable $DBParams -serverinstance "ipc-vsql01" -database "DataAgg" 
