...
class perform:
    ...
    def ReadAndLoad(self):
        ...
        try:
            ...
            #print 'got here !!',filedatetime
            from datetime import datetime
            filedatetime_forsql = datetime.fromtimestamp(filedatetime).strftime('%Y-%m-%d %H:%M:%S')
            #print 'got here !!!',filedatetime_forsql
            
            import bs4, sys
            print '&&&&', my_datafilepathname
            ...
            #MasterPortfolioStateCode	PortfolioRoot	MoneyManager	client_subgroup	PortfolioId	PortfolioCode	PortfolioName	CustodianName	TerritoryCode	CustodianAccountNumber	TaxFilingNumber	InitializationDate	ManagementDate	PortfolioEffectiveDate	IsMasterFlag	IsActiveFlag	TaxSensitivityCode	InvestmentObjectiveDescription	InvestmentDiscretionDescription	AccountTypeDescription	RRDescription	PortfolioStateCode	InvesmentEntityCode	AccountSubTypeCode	AccountSubTypeDescription	TerminationDate	investment_objective_code	ValueOfSecurities	ValueOfAccruedIncome	ValueOfCash	AcctType	correspondence_firm	correspondence_firm_name	TotalMarketValue	AsOfDate
            ...
            for nodeA in soup.find_all('tr', attrs={}):
                ...
                for nodeB in nodeA.find_all('td', attrs={}):
                    ...
                    if nodeB.attrs['class'][0] in ['DataCellNumeric','DataCellString']:
                        ...
                        if fieldnames[len(fieldvalues)] == 'MasterPortfolioStateCode':
                            is_dataline = 1
                            row_id = nodeB.string
                            print 'MasterPortfolioStateCode:',row_id
                ...
                if is_dataline == 1:
                    ...
                    if already_ran_delete_statement == 0:
                        cursor.execute("delete from dbo.aumoriginalcodeblock_test01 where AsOfDate = ?", fieldvalues['AsOfDate'] )
                        total_deleted = total_deleted + cursor.rowcount
                        print '  ',cursor.rowcount, 'records deleted'
                        cnxn.commit()
                        already_ran_delete_statement = 1
                    insert_sql = "insert into aumoriginalcodeblock_test01("+fieldnames_string+") values ("+fieldvalues_string+")"
                    #print insert_sql
...
if __name__=='__main__':
    ...
    #myDataFilePathName = r"\\Ipc-vsql01\data\Batches\prod\WatchFolder\incoming\PagesOutput_AUMOriginalCodeBlock_20161209 134614.xls"
    myDataFilePathName = r"//Ipc-vsql01/data/Batches/prod/WatchFolder/incoming/PagesOutput_AUMOriginalCodeBlock_20161209 134614.xls"
    o = perform(myDataFilePathName)
    #o.DataFilePathName = r"//Ipc-vsql01/data/Batches/prod/WatchFolder/incoming/PagesOutput_AUMOriginalCodeBlock_20161209 134614.xls"
    #o.ReadAndLoad()
    print o.Results
