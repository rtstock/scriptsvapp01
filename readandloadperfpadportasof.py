        ...
        try:
            ...
            for nodeA in soup.find_all('tr', attrs={}):
                ...
                if is_dataline == 1:
                    ...
                    cursor = cnxn.cursor()
                    #sql_delete = "delete from dbo.xanalysisofbenchmarks_padportasof_imported where class_last_invested_date = ? and portfolio_ext = ?", fieldvalues['class_last_invested_date'],fieldvalues['portfolio_ext']
                    #print sql_delete
                    cursor.execute("delete from dbo.xanalysisofbenchmarks_padportasof_imported where class_last_invested_date = ? and portfolio_ext = ?", fieldvalues['class_last_invested_date'],fieldvalues['portfolio_ext']  )
                    total_deleted = total_deleted + cursor.rowcount
                    print '  ',cursor.rowcount, 'records deleted'
                    cnxn.commit() 
                    insert_sql = "insert into xanalysisofbenchmarks_padportasof_imported("+fieldnames_string+") values ("+fieldvalues_string+")"
                    print insert_sql
...
if __name__=='__main__':
    ...
    #myDataFilePathName = r"//Ipc-vsql01/data/Batches/prod/WatchFolder/incoming/PagesOutput_GetPPAsOf_2016-11-29 152725403.xls"
    myDataFilePathName = r"//Ipc-vsql01/data/Batches/prod/WatchFolder/incoming/xPagesOutput_GetPPAsOf_2017-02-14 072627403.xls"
